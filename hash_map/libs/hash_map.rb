...
class HashMap # rubocop:disable Metrics/ClassLength
  attr_reader :buckets, :utilization
  ...
  def set(key, value) # rubocop:disable Metrics/AbcSize,Metrics/MethodLength
    index = hash(key)
    if @buckets[index].nil?
      ...
      while pointer
        ...
        @buckets[index].append(key, value) if pointer.nil?
      end
    end
  ...
  def get(key)
    ...
    pointer = @buckets[index].head
    while pointer
      return pointer.value if pointer.key == key
  ...
  def values
    ...
    @buckets.each do |list|
      ...
      pointer = list.head
      while pointer
        @values << pointer.value unless pointer.nil?
        pointer = pointer.next_node
      end
    end
    @values
  ...
  def entries # rubocop:disable Metrics/MethodLength
    ...
    @buckets.each do |list|
      ...
      while pointer
        current_key_value = []
        (current_key_value << pointer.key) && (current_key_value << pointer.value) unless pointer.nil?
        @entries << current_key_value unless current_key_value == []
